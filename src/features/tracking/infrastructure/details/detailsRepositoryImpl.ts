...
import { ConciliationRouteResponse, DataUploadFIlePprops, EvidenciaOTPItemOTP, ListInfOTP, NoveltyRefusedProps, PaymentsByInvoicePorps, ReentryOTPProps, ReportNoveltyFileArrayProps, RuteInitPorps, SendOrderArrayProps, SendOrderProps, SendOTOPProps, ValidateCediQRResponse, ValidateCodeOTPProps, ValidateOrderProps } from "../../domain/details/DetailsGuide";
...
export const detailsRepositoryImpl: DetailsRepository = {
  ...
  async sendValidateOrder(data: ValidateOrderProps, token: string) {
    try {
      const response = await authApi.post(API_ROUTES.VALIDATE_ORER, data, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
